...
switch (invocation.mode) {
	case "profile": {
		const { runProfile } = await import("./profile-boot-Bnhj3MXs.js");
		await runProfile({
	...
	case "plugin": {
		const { runPlugin } = await import("./plugin-DIhBp0rC.js");
		process.exit(runPlugin(invocation.profile, invocation.args));
		break;
	}
	case "dump-config": {
		const { runDumpConfig } = await import("./dump-config-DVyiPB6d.js");
