// vendor/harness/packages/terminal/terminal-bash/src/index.ts
import { TerminalBackendCleanupError } from "@deepseek-ai/dsh-terminal";
import { effectiveSandboxMode } from "@deepseek-ai/dsh-sandbox-policy";

// vendor/harness/packages/terminal/terminal-bash/src/config.ts
...
function validateConfig(config) {
  ...
  for (const [name2, value] of Object.entries(resolved)) {
    if (typeof value === "number" && (!Number.isSafeInteger(value) || value <= 0)) {
      throw new Error(`terminal-bash: ${name2} must be a positive safe integer`);
    }
  }
...
// vendor/harness/packages/terminal/terminal-bash/src/session.ts
import { Buffer as Buffer2 } from "node:buffer";
import { TerminalError } from "@deepseek-ai/dsh-terminal";

// vendor/harness/packages/terminal/terminal-bash/src/sanitize.ts
import { Buffer } from "node:buffer";
var PROMPT_MARKER_PREFIX = "133;D;";
var CONTROLLED_PROMPT = "dsh> ";
...
`;
  ...
  discardPrefix(chunk) {
    ...
    if (this.discardMode === "csi") {
      for (let index2 = 0; index2 < chunk.length; index2 += 1) {
        const code = chunk.charCodeAt(index2);
        if (code >= 64 && code <= 126) {
          this.discardMode = void 0;
          return chunk.slice(index2 + 1);
        }
      }
      return "";
    }
...
function utf8Tail(text, maxBytes) {
  if (Buffer2.byteLength(text) <= maxBytes) return { text, truncated: false };
  const chars = Array.from(text);
  let bytes = 0;
  let start = chars.length;
  while (start > 0) {
    const next = Buffer2.byteLength(chars[start - 1]);
...
var LocalPtySession = class {
  ...
  async initialize(signal) {
    ...
    try {
      const operation = this.startSend({ text: "", submit: false, ...signal !== void 0 ? { signal } : {} });
  ...
  startSend(request) {
    ...
    this.activeDeadlineTimer = setTimeout(() => {
      if (this.active === operation) {
        this.settleActive("timeout", this.activeWrite !== void 0 || this.interrupting === operation);
      }
  ...
  async pollReadiness(operation) {
    ...
    try {
      ...
      if (this.promptSeen && foreground !== void 0 && this.shellPgid === void 0) {
...
// vendor/harness/packages/terminal/terminal-bash/src/index.ts
var name = "terminal-bash";
var inject = ["terminals", "sandboxPolicy", "subprocess"];
var sandboxModeFences = /* @__PURE__ */ new WeakMap();
...
var BashTerminalBackend = class {
  constructor(ctx, config, spawnTerminal = (spec) => ctx.subprocess.spawnTerminal(spec), createSession = (terminal, config2) => new LocalPtySession(terminal, config2)) {
    this.ctx = ctx;
    this.config = config;
