// vendor/harness/packages/subagent/tool-subagent/src/index.ts
import z from "@deepseek-ai/schemastery";
import { defineTool } from "@deepseek-ai/dsh-tools";
import { assertSubagentMaxDepth, settleRun } from "@deepseek-ai/dsh-subagent";
var name = "tool-subagent";
...
async function settleStart(start, signal) {
  try {
    ...
    return signal.aborted && !(error instanceof AggregateError) ? { status: "killed" } : { status: "failed", detail: String(error) };
  }
}
...
function apply(ctx, config) {
  ...
  const mount = (provider) => {
    if (typeof config.maxDepth === "number" && !provider.capabilities.depthLimit) {
      throw new Error(
        `tool-subagent: provider "${provider.name}" cannot enforce maxDepth (no depthLimit capability) \u2014 set maxDepth: 'provider-managed' to leave the recursion budget to the provider`
      );
    }
