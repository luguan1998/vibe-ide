// vendor/harness/packages/extensions/tool-cordis/src/index.ts
import {
...
// vendor/harness/packages/extensions/tool-cordis/src/api-catalog.ts
var SERVICE_API = [
  {
...
function queryEventApi(name2, events = EVENT_API) {
  if (name2 === void 0) {
    return {
      mode: "catalog",
  ...
  const event = events.find((candidate) => candidate.name === name2);
  if (event === void 0) throw new Error(`no catalogued Event named "${name2}"`);
  return {
    mode: "event",
    event: {
      name: event.name,
...
// vendor/harness/packages/extensions/tool-cordis/src/fiber-state.ts
var FiberState = {
  PENDING: 0,
  LOADING: 1,
  ACTIVE: 2,
...
// vendor/harness/packages/extensions/tool-cordis/src/inspect.ts
...
// vendor/harness/packages/extensions/tool-cordis/src/present.ts
...
// vendor/harness/packages/extensions/tool-cordis/src/prompt.ts
var CORDIS_SYSTEM_PROMPT = `# Dynamic Cordis Plugins
...
// vendor/harness/packages/extensions/tool-cordis/src/providers.ts
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args, exec) {
      const plugin = args.plugin.kind === "new" ? { kind: "new", idPrefix: args.plugin.idPrefix } : { kind: "existing", pluginId: CordisDynamicPluginId(args.plugin.pluginId) };
