...
export async function createBranchWorktree(cwd: string, configDir?: string): Promise<BranchWorktree | { error: string }> {
  ...
  const wtPath = join(repoRoot, marker, 'branch-worktrees', id)
  const indexFile = join(tmpdir(), `vibe-wt-${id}.index`)
...
export function registerWorktreeHandlers(): void {
  ipcMain.handle(IPC_CHANNELS.AI_CREATE_BRANCH_WORKTREE, async (_e, payload: { sessionId: string; cwd?: string; configDir?: string }) => {
    const live = aiSessions.get(payload.sessionId)
    const cwd = live?.cwd || payload.cwd
    if (!cwd) return { error: 'No workspace path' }
    const result = await createBranchWorktree(cwd, live?.configDir ?? payload.configDir)
