...
const api = {
  ...
  terminal: {
    ...
    refreshEnv: () => ipcRenderer.invoke(IPC_CHANNELS.PTY_REFRESH_ENV),
  ...
  git: {
    ...
    pull: (remote?: string, branch?: string, cwd?: string) => ipcRenderer.invoke(IPC_CHANNELS.GIT_PULL, remote, branch, cwd),
    remoteBranches: (cwd?: string) => ipcRenderer.invoke(IPC_CHANNELS.GIT_REMOTE_BRANCHES, cwd),
    ...
    removeMetaChangedListener: (handler?: any) => {
      if (handler) {
        ipcRenderer.removeListener(IPC_CHANNELS.GIT_META_CHANGED, handler)
      } else {
        ipcRenderer.removeAllListeners(IPC_CHANNELS.GIT_META_CHANGED)
      }
    }
  },
  ...
  ai: {
    ...
    removeReplyListener: (handler?: any) => {
      if (handler) ipcRenderer.removeListener(IPC_CHANNELS.AI_REPLY, handler)
      else ipcRenderer.removeAllListeners(IPC_CHANNELS.AI_REPLY)
    },
  },
