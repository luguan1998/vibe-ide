...
export default function OutlineTrigger({ content, filePath, fullPath, onNavigate }: OutlineTriggerProps) {
  ...
  return (
    <div className="relative" onMouseEnter={clearHide} onMouseLeave={scheduleHide}>
      <button
        onClick={() => { clearHide(); setShow(prev => !prev) }}
        className={`w-6 h-6 flex items-center justify-center rounded transition-colors ${
          show ? 'text-ide-accent bg-ide-accent/10' : 'text-ide-text-muted hover:text-ide-text hover:bg-ide-hover'
        }`}
        title="Outline"
      >
      ...
      {show && (
        <div className="absolute top-full right-0 mt-1 z-50 w-[280px] max-h-[60vh] flex flex-col overflow-hidden rounded-lg border border-ide-border bg-ide-sidebar/70 backdrop-blur-md shadow-lg">
          <OutlinePanel
            filePath={filePath}
            fullPath={fullPath}
            content={content}
